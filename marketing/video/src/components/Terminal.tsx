...
export const Terminal: React.FC<{
  ...
  width?: number;
  /** Monospace font size for body text. Defaults to 24. Reduce for portrait. */
  fontSize?: number;
  showCursor?: boolean;
}> = ({ title = "claude code", lines, visibleLines, width = 980, fontSize = 24, showCursor = true }) => {
  return (
    <div
      style={{
        width,
    ...
    >
      ...
      {/* body */}
      <div style={{ padding: "26px 30px", fontFamily: MONO, fontSize, lineHeight: 1.7 }}>
        {lines.slice(0, visibleLines).map((line, i) => (
          <div key={i} style={{ color: lineColor(line.kind), display: "flex" }}>
            ...
            {line.kind === "claude" && (
              <span style={{ color: COLORS.amberDeep, marginRight: 12, display: "inline-flex", alignItems: "center" }}>
