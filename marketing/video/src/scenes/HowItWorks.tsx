...
const Sub: React.FC<{
  ...
  return (
    <AbsoluteFill
      style={{ opacity, transform: `scale(${0.98 + opacity * 0.02})` }}
    >
      {children}
    </AbsoluteFill>
  );
...
const Caption: React.FC<{ children: React.ReactNode; delay?: number }> = ({
  ...
  <div style={{ position: "absolute", top: 90, width: "100%", textAlign: "center" }}>
    <FadeUp delay={delay}>
      <div style={{ fontSize: 46, fontWeight: 800, color: COLORS.ink, fontFamily: FONT }}>
        {children}
      </div>
    </FadeUp>
  </div>
...
// A compact inline "🪄 Lumi — quick lesson" snippet for terminal/mobile faces.
const InlineLesson: React.FC<{ concept: string; body: string; size?: number }> = ({
  ...
  >
    ...
    <div style={{ fontSize: size * 0.92, color: COLORS.inkSoft, lineHeight: 1.45, marginTop: 2 }}>
      {body}
    </div>
  </div>
);
...
export const HowItWorks: React.FC = () => {
  ...
  const cardIn = interpolate(frame, [56, 72], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const cardReveal = interpolate(frame, [70, 104], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // Beat C: shelf fill + the "stay quiet" repeat note
  const shelfRevealed = interpolate(frame, [222, 286], [0, SHELF.length], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const quietIn = interpolate(frame, [262, 280], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
    ...
    <AbsoluteFill style={{ fontFamily: FONT }}>
        ...
        >
          ...
          <Surface label="Terminal" delay={130}>
            ...
            >
              ...
              <InlineLesson
                concept="Migration"
                body="A safe, step-by-step update to your database's shape — like a recipe it can re-run."
              />
            </div>
          </Surface>
          ...
          <Surface label="Mobile" delay={142}>
            <Phone width={300}>
              ...
              <InlineLesson
                concept="API"
                body="A doorway one app uses to ask another for data or actions."
                size={15}
              />
            </Phone>
          </Surface>
      ...
      <Sub start={210} end={300}>
        ...
        <div style={{ position: "absolute", left: "50%", top: 300, transform: "translateX(-50%)" }}>
          <ProgressShelf concepts={SHELF} revealed={shelfRevealed} width={1180} />
        </div>
        <div
          style={{
            position: "absolute",
        ...
        >
          ...
          <span style={{ fontSize: 26, color: COLORS.inkSoft }}>
            Seen <b style={{ color: COLORS.teal }}>“API”</b> again? Lumi stays quiet — no
            repeats, no overwhelm.
          </span>
        </div>
      </Sub>
    </AbsoluteFill>
